...
class EvalRequestServiceImplSpec extends Specification {
    ...
    def "test eval method - normal execution - userId: #userId, eventId: #eventId, traceId: #traceId"() {
        ...
        def expectedValue = new TypedValue(true, ValueTypeEnum.BOOLEAN)
        def trace = new ExecutionTrace()
        trace.setTraceId(traceId as BigInteger)
        def expectedResult = new EvalResult(expectedValue, trace)

        when: "execute eval method"
        def evalResult = evalService.eval(request, BigInteger.ONE)
        ...
        evalResult != null
        evalResult.value == expectedResult.value
    ...
    def "test eval method - kafka send failure should not affect main flow - errorMessage: #errorMessage"() {
        ...
        def expectedValue = new TypedValue(true, ValueTypeEnum.BOOLEAN)
        def trace = new ExecutionTrace()
        trace.setTraceId(BigInteger.ZERO)
        def expectedResult = new EvalResult(expectedValue, trace)
    ...
    def "test eval method - verify execution context construction - userId: #userId, eventId: #eventId, traceId: #traceId, argKey: #argKey, argValue: #argValue"() {
        ...
        when: "execute eval method"
        evalService.eval(request, BigInteger.ONE)
